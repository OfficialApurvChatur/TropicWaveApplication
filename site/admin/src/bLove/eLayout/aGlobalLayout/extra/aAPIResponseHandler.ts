...
const apiResponseHandler = {
  userAccountRetrieveAPIResponseHandler: (ReduxCall: any, retrieveAPIResponse: any) => {
    // Handle loading
    ...
    if (retrieveAPIResponse.isError) {
      if (retrieveAPIResponse.error && retrieveAPIResponse.error.originalStatus === 404) {
        ...
        return toast({
          variant: "destructive",
          title: "Uh oh! Can't find account.",
          description: "Seems... You are not signed in bruh...",
        });
      } else {
        return toast({
          variant: "destructive",
    ...
    if (retrieveAPIResponse.isSuccess && retrieveAPIResponse.data?.success) {
      ReduxCall.dispatch(
        ReduxCall.action.receivedObjectAction({
          AccountRetrieve: {
            ...retrieveAPIResponse.data.user_account_retrieve,
