...
import { AccessPointModel } from '../../../aModel/admin/bUserAdministration/aAccessPointModel';


const accessPointController = (Model=AccessPointModel, Label="AccessPoint") => ({
  // List
  ...
  create: catchAsyncMiddleware(
    async (request: express.Request, response: express.Response, next: express.NextFunction) => {
      ...
      // Clear Cache
      await redisClient.del(`${Label.toLowerCase()}-list`, `${Label.toLowerCase()}-list-for-menu-create-and-update`);
  ...
  listForMenuCreateAndUpdate: catchAsyncMiddleware(
    async (request: express.Request, response: express.Response, next: express.NextFunction) => {
      ...
      // Set Cache
      await redisClient.setex(`${Label.toLowerCase()}-list-for-menu-create-and-update`, 15*60, JSON.stringify(list));
